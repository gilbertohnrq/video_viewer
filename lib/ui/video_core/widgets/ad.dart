...
class VideoCoreAdViewer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomOpacityTransition(
      ...
      child: Stack(
        children: [
          video.activeAd?.child ?? const SizedBox(),
          if (video.activeAd != null)
            Align(
              alignment: style.skipAdAlignment,
              child: SplashTap(
                onTap: (video.adTimeWatched ?? Duration.zero) >= video.activeAd!.durationToSkip ? video.skipAd : null,
                child: Builder(builder: (_) {
                  final int remaing = (video.activeAd!.durationToSkip - (video.adTimeWatched ?? Duration.zero)).inSeconds;
                  return style.skipAdBuilder?.call(video.adTimeWatched!) ??
                      Container(
                        ...
                        child: Row(mainAxisSize: MainAxisSize.min, children: [
                          ...
                          if (remaing <= 0) const Icon(Icons.skip_next, color: Colors.white)
                        ]),
                      );
                }),
              ),
